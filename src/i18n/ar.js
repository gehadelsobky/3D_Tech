export default {
  // Navigation
  nav: {
    home: 'الرئيسية',
    services: 'خدماتنا',
    products: 'المنتجات',
    giftFinder: 'اختيار الهدايا',
    about: 'من نحن',
    contact: 'طلب عرض سعر',
    privacy: 'سياسة الخصوصية',
    blog: 'المقالات',
    requestQuote: 'طلب عرض سعر',
  },

  // Home Page
  home: {
    heroTitle: 'طباعة ثلاثية الأبعاد وهدايا مؤسسية مميزة',
    heroSubtitle: 'منتجات ترويجية مطبوعة بتقنية 3D وهدايا مخصصة لكل مناسبة.',
    exploreCta: 'استكشف المنتجات',
    quoteCta: 'طلب عرض سعر',
    featuredTitle: 'منتجات مميزة',
    viewAll: 'عرض جميع المنتجات',
    whyTitle: 'لماذا 3DTECH؟',
    howTitle: 'كيف نعمل',
    categoriesTitle: 'تصفح حسب الفئة',
    clientsTitle: 'يثق بنا كبار الشركات',
  },

  // Products Page
  products: {
    title: 'منتجاتنا',
    subtitle: 'تصفح مجموعتنا من المنتجات الترويجية المطبوعة بتقنية 3D',
    searchPlaceholder: 'ابحث عن منتج...',
    allCategories: 'الكل',
    noResults: 'لا توجد منتجات.',
    noResultsDescription: 'جرّب تعديل معايير البحث أو التصفية',
    showingOf: 'عرض {count} من {total} منتج',
    moq: 'الحد الأدنى للطلب',
    units: 'قطعة',
    relatedTitle: 'منتجات مشابهة',
  },

  // Product Detail
  productDetail: {
    priceRange: 'نطاق السعر',
    minOrder: 'الحد الأدنى للطلب',
    leadTime: 'مدة التسليم',
    notes: 'ملاحظات',
    features: 'المميزات',
    brandingOptions: 'خيارات العلامة التجارية',
    requestItem: 'طلب هذا المنتج',
  },

  request: {
    title: 'طلب هذا المنتج',
    subtitle: 'أدخل بياناتك وسنتواصل معك خلال 24 ساعة بعرض سعر مخصص.',
    backToProduct: 'العودة للمنتج',
    productDetails: 'تفاصيل المنتج',
    yourDetails: 'بياناتك',
    quantity: 'الكمية',
    quantityPlaceholder: 'مثال: 50 قطعة',
    deliveryDate: 'تاريخ التسليم المفضل',
    notes: 'ملاحظات إضافية',
    notesPlaceholder: 'أي متطلبات للعلامة التجارية أو طلبات خاصة أو أسئلة...',
    submit: 'إرسال الطلب',
    submitting: 'جارٍ الإرسال...',
    successTitle: 'تم إرسال الطلب!',
    successMessage: 'شكراً {name}! تلقينا طلبك لـ {product} وسنرسل لك عرض سعر مخصص خلال 24 ساعة.',
    successBack: 'العودة للمنتجات',
    sendAnother: 'إرسال طلب آخر',
  },

  // Services Page
  services: {
    heroTitle: 'خدماتنا',
    heroSubtitle: 'من الطباعة ثلاثية الأبعاد إلى حلول الهدايا المؤسسية، نحول أفكارك إلى واقع.',
    printingTitle: 'خدمات الطباعة ثلاثية الأبعاد',
    giftsTitle: 'حلول الهدايا المؤسسية',
    ctaTitle: 'مستعد لبدء مشروعك؟',
    ctaButton: 'طلب عرض سعر',
  },

  // About Page
  about: {
    heroTitle: 'عن 3D Tech',
    vision: 'رؤيتنا',
    mission: 'مهمتنا',
    valuesTitle: 'قيمنا الأساسية',
    statsTitle: 'إنجازاتنا',
    whyTitle: 'لماذا تختار 3DTECH؟',
    howTitle: 'كيف نعمل',
    ctaTitle: 'مستعد لإنشاء شيء مميز؟',
    ctaButton: 'طلب عرض سعر',
  },

  // Contact Page
  contact: {
    title: 'طلب عرض سعر',
    subtitle: 'املأ النموذج أدناه وسنعاود الاتصال بك خلال 24 ساعة.',
    name: 'الاسم الكامل',
    namePlaceholder: 'أحمد محمد',
    company: 'اسم الشركة',
    companyPlaceholder: 'اسم الشركة',
    email: 'البريد الإلكتروني',
    emailPlaceholder: 'ahmed@example.com',
    phone: 'رقم الهاتف',
    phonePlaceholder: '+20 10XXXXXXXX',
    giftType: 'نوع الهدية',
    selectGiftType: 'اختر نوع الهدية...',
    product: 'المنتج المطلوب',
    selectProduct: 'اختر منتج (اختياري)...',
    quantity: 'الكمية',
    quantityPlaceholder: 'مثال: 100 قطعة',
    budget: 'نطاق الميزانية',
    budgetPlaceholder: 'مثال: 5,000 - 10,000 جنيه',
    selectBudget: 'اختر نطاق الميزانية...',
    deliveryDate: 'تاريخ التسليم',
    message: 'ملاحظات إضافية',
    messagePlaceholder: 'أخبرنا عن مشروعك واحتياجات العلامة التجارية أو أي متطلبات خاصة...',
    submit: 'إرسال الطلب',
    submitting: 'جارٍ الإرسال...',
    successTitle: 'شكراً لك!',
    successMessage: 'تم إرسال طلبك بنجاح. سنعاود الاتصال بك قريباً.',
    submitAnother: 'إرسال طلب آخر',
    contactInfo: 'معلومات الاتصال',
    requestingFor: 'طلب عرض سعر لـ:',
    emailLabel: 'البريد الإلكتروني',
    phoneLabel: 'الهاتف',
    locationLabel: 'الموقع',
    whyUs: 'لماذا {company}؟',
    errNameRequired: 'الاسم مطلوب',
    errEmailRequired: 'البريد الإلكتروني مطلوب',
    errEmailInvalid: 'صيغة البريد الإلكتروني غير صحيحة',
    errPhoneRequired: 'رقم الهاتف مطلوب',
  },

  // Gift Finder
  giftFinder: {
    title: 'اختيار الهدايا',
    subtitle: 'أجب على بعض الأسئلة وسنقترح عليك أفضل المنتجات المناسبة لاحتياجاتك.',
    next: 'التالي',
    back: 'رجوع',
    findGifts: 'ابحث عن الهدايا',
    startOver: 'ابدأ من جديد',
    resultsTitle: 'المنتجات الموصى بها',
    approxResultsTitle: 'أقرب الخيارات',
    approxBadge: 'قريب من طلبك',
    approxOverBudget: 'أعلى من الميزانية التي اخترتها',
    approxSlowerDelivery: 'مدة التسليم أطول من المطلوب',
    noMatchTitle: 'لم نجد مطابقة دقيقة',
    noMatchText: 'أخبرنا باحتياجك وسيقترح عليك فريقنا المنتجات المناسبة لميزانيتك وموعدك.',
    noMatchCta: 'تواصل معنا',
    noResults: 'لم يتم العثور على منتجات مطابقة. جرّب فلاتر مختلفة.',
    requestQuote: 'اطلب عرض سعر',
    stepOf: 'الخطوة {current} من {total}',
    yourSelections: 'اختياراتك',
    recommendedCategories: 'الفئات الموصى بها',
    purpose: 'الغرض',
    budget: 'الميزانية',
    audience: 'الجمهور',
    quantity: 'الكمية',
    timeline: 'الجدول الزمني',
    qPurpose: 'ما هو الغرض من الهدية؟',
    qBudget: 'ما هي ميزانيتك للقطعة الواحدة؟',
    qAudience: 'من هو الجمهور المستهدف؟',
    qQuantity: 'كم عدد القطع المطلوبة؟',
    qTimeline: 'متى تحتاجها؟',
    options: {
      'Corporate Event': 'فعالية مؤسسية',
      'Trade Show / Expo': 'معرض تجاري',
      'Employee Appreciation': 'تقدير الموظفين',
      'Client Gift': 'هدية للعملاء',
      'Product Launch': 'إطلاق منتج',
      'Holiday / Seasonal': 'مناسبات موسمية',
      'Wedding / Party Favor': 'أفراح وحفلات',
      'Other': 'أخرى',
      'Under EGP 100 per unit': 'أقل من 100 جنيه للقطعة',
      'EGP 100 - EGP 500 per unit': '100 - 500 جنيه للقطعة',
      'EGP 500 - EGP 1000 per unit': '500 - 1000 جنيه للقطعة',
      'EGP 1000+ per unit': 'أكثر من 1000 جنيه للقطعة',
      'Corporate / B2B': 'شركات / B2B',
      'General Public': 'الجمهور العام',
      'Students / Education': 'طلاب / تعليم',
      'Tech Enthusiasts': 'عشاق التكنولوجيا',
      'Employees / Internal': 'موظفين / داخلي',
      '20 - 50 units': '20 - 50 قطعة',
      '50 - 100 units': '50 - 100 قطعة',
      '100 - 500 units': '100 - 500 قطعة',
      '500+ units': '+500 قطعة',
      'Within 1 week': 'خلال أسبوع',
      '1-2 weeks': '1-2 أسبوع',
      '2-4 weeks': '2-4 أسابيع',
      '1 month+': 'شهر أو أكثر',
    },
  },

  // Blog
  blog: {
    title: 'المقالات',
    subtitle: 'أحدث المقالات والنصائح والرؤى حول الطباعة ثلاثية الأبعاد والهدايا المؤسسية.',
    noArticles: 'لا توجد مقالات بعد',
    checkBack: 'تابعنا قريباً لمحتوى جديد.',
    backToBlog: 'العودة للمقالات',
  },

  // Privacy Page
  privacy: {
    title: 'سياسة الخصوصية',
  },

  // Login
  login: {
    title: 'تسجيل دخول المشرف',
    subtitle: 'سجل الدخول لإدارة المنتجات',
    username: 'اسم المستخدم',
    usernamePlaceholder: 'أدخل اسم المستخدم',
    password: 'كلمة المرور',
    passwordPlaceholder: 'أدخل كلمة المرور',
    submit: 'تسجيل الدخول',
    submitting: 'جارٍ تسجيل الدخول...',
    forgot: 'نسيت كلمة المرور؟',
    backToLogin: '→ العودة لتسجيل الدخول',
    forgotTitle: 'إعادة تعيين كلمة المرور',
    forgotSubtitle: 'أدخل البريد الإلكتروني المرتبط بحسابك وسنرسل لك رابط إعادة التعيين.',
    forgotEmail: 'البريد الإلكتروني',
    forgotEmailPlaceholder: 'you@company.com',
    forgotSubmit: 'أرسل رابط الإعادة',
    forgotSending: 'جارٍ الإرسال...',
    forgotSentTitle: 'تفقّد بريدك',
    forgotSentText: 'إذا كان هذا البريد مرتبطاً بحساب مشرف، فرابط الإعادة في الطريق. ينتهي الرابط خلال {minutes} دقيقة.',
    resetTitle: 'اختر كلمة مرور جديدة',
    resetFor: 'إعادة تعيين كلمة المرور لـ {username}.',
    resetPassword: 'كلمة المرور الجديدة',
    resetPasswordPlaceholder: 'لا تقل عن 8 أحرف',
    resetConfirm: 'تأكيد كلمة المرور',
    resetConfirmPlaceholder: 'أعد كتابة كلمة المرور',
    resetSubmit: 'حفظ كلمة المرور',
    resetSubmitting: 'جارٍ الحفظ...',
    resetChecking: 'جارٍ التحقق من الرابط...',
    resetInvalidTitle: 'هذا الرابط لم يعد صالحاً',
    resetInvalidText: 'روابط الإعادة تنتهي بعد {minutes} دقيقة وتُستخدم مرة واحدة فقط. اطلب رابطاً جديداً.',
    resetRequestNew: 'اطلب رابطاً جديداً',
    resetDoneTitle: 'تم تحديث كلمة المرور',
    resetDoneText: 'يمكنك الآن تسجيل الدخول بكلمة المرور الجديدة.',
    resetGoToLogin: 'اذهب لتسجيل الدخول',
    errPasswordShort: 'كلمة المرور يجب ألا تقل عن 8 أحرف',
    errPasswordMismatch: 'كلمتا المرور غير متطابقتين',
    errEmailRequired: 'البريد الإلكتروني مطلوب',
  },

  // Footer
  footer: {
    productsTitle: 'المنتجات',
    companyTitle: 'الشركة',
    getInTouch: 'تواصل معنا',
    rights: 'جميع الحقوق محفوظة.',
    privacy: 'الخصوصية',
    contact: 'اتصل بنا',
  },

  // Common
  common: {
    loading: 'جارٍ التحميل...',
    error: 'حدث خطأ ما',
    retry: 'حاول مرة أخرى',
    save: 'حفظ',
    cancel: 'إلغاء',
    delete: 'حذف',
    edit: 'تعديل',
    add: 'إضافة',
    search: 'بحث',
    close: 'إغلاق',
    confirm: 'تأكيد',
    yes: 'نعم',
    no: 'لا',
    viewAll: 'عرض الكل',
    learnMore: 'اعرف المزيد',
    backToHome: 'العودة للرئيسية',
  },

  // 404 Page
  notFound: {
    title: 'الصفحة غير موجودة',
    message: 'الصفحة التي تبحث عنها غير موجودة أو تم نقلها.',
    backHome: 'العودة للرئيسية',
  },

  // Error Boundary
  errorBoundary: {
    title: 'حدث خطأ ما',
    message: 'حدث خطأ غير متوقع. يرجى تحديث الصفحة.',
    refresh: 'تحديث الصفحة',
  },

  productImport: {
    button: 'استيراد CSV',
    title: 'استيراد منتجات',
    intro: 'ابدأ بالقالب حتى تتطابق الأعمدة.',
    downloadTemplate: '⬇ تحميل قالب CSV',
    chooseFile: 'اختر ملفاً',
    validate: 'فحص الملف',
    validating: 'جارٍ الفحص...',
    cancel: 'إلغاء',
    importing: 'جارٍ الاستيراد...',
    importAction: 'استورد {count} منتج',
    importActionOne: 'استورد {count} منتج',
    summary: '{total} صف · {valid} جاهز · {errors} فيها أخطاء',
    unknownColumn: 'عمود غير معروف "{column}" — سيتم تجاهله',
    previewTitle: 'أول المنتجات التي ستضاف:',
    nothingValid: 'لم يجتز أي صف الفحص. صحح الملف وحاول مرة أخرى.',
    done: 'تم استيراد {count} منتج.',
    doneOne: 'تم استيراد {count} منتج.',
    rowLabel: 'صف',
    importAnother: 'استيراد ملف آخر',
    templateDownloadError: 'تعذر تنزيل القالب',
    requestFailed: 'فشل الطلب',
  },
};
