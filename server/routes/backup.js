...
router.post('/', authenticate, requirePermission('settings.smtp'), async (_req, res) => {
  try {
...
router.get('/list', authenticate, requirePermission('settings.smtp'), (_req, res) => {
  try {
    const backups = readdirSync(BACKUP_DIR)
      .filter(f => f.startsWith('backup_') && f.endsWith('.db'))
      .map(f => {
...
router.get('/download/:name', authenticate, requirePermission('settings.smtp'), (req, res) => {
